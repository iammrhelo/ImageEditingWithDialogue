...
python run.py vocab -t ${illc_dir}/train.txt -o ${illc_dir}/vocab.bin 

echo "Step 2: train LSTM tagger"
python run.py train --train ${illc_dir}/train.txt \
                    --valid ${illc_dir}/valid.txt \
                    --vocab ${illc_dir}/vocab.bin \
                    --work-dir ${work_dir}

echo "Step 3: eval tagger"
python run.py eval --work-dir ${work_dir} \
                    --tag ${illc_dir}/test.txt 
